  ...
  return Promise.resolve()
    ...
    .then(() => {
      if (hasChildren) {
        ...
        return Promise.all(layers.map(l => parseLayer(zip, l, newPath, sharedStyles, recursive)))
      }
    }).then(() => {
      if (hasChildren) {
        return fs.writeFile(
...
function parsePage (zip, path, options, sharedStyles, file) {
  return zip.file(file).async('string').then((data) => {
    ...
    return fs.mkdir(newPath).catch((err) => {
      ...
      Promise.all(layers.map(l => parseLayer(zip, l, newPath, sharedStyles, !options.limitToTopLevelLayers)))
    ).then(() => fs.writeFile(
