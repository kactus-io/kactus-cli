...
module.exports = function (path, options) {
  ...
  return importStyles(path, options).then(sharedStyles => {
    ...
    return Promise.all([
      fs.readdir(path)
        ...
        .then(files => Promise.all(
          (options.sharedPages || []).map(name => importPage(zip, options.root, name, stylesMap, onLibraryPreviewFound)).concat(
            files.map(f => importPage(zip, path, f, stylesMap, onLibraryPreviewFound))
          )
        ))
    ...
    .then(() => generatePreviews(zip, path + '.sketch', options.sketchPath, libraryPreview))
    .then((generatedPreviews) => generatedPreviews ? fsZip.writeZipToFile(zip, path + '.sketch') : undefined)
    .then(() => path + '.sketch')
}
