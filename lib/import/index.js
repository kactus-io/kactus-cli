...
module.exports = function (path, options) {
  ...
  return importStyles(path, options).then(sharedStyles => {
    const stylesMap = importStyles.mapStyles(sharedStyles)
    return Promise.all([
      fs.readdir(path)
        ...
        .then(files => Promise.all(
          files.map(f => importPage(zip, path, f, stylesMap)).concat(
          (options.sharedPages || []).map(name => importPage(zip, options.root || Path.dirname(path), name, sharedStyles))
          )
        ))
